...
gadget_files = ['snfs_gadgets.txt', 'csncdav6_gadgets.txt', 'csmtpav6_gadgets.txt', 'csftpav6_gadgets.txt'] #File(s) of Gadgets, written with rp++ output
...
for gadget_file in gadget_files:
	for line in open(gadget_file).readlines():
		if "0x" in line:
...
for g in ropchain:
	if "bypass" in g:
		print("ropchain += pack('<L', (%s)) %s %s [Line: %d]" % (g.split(' ')[1], pound, g, linenumber))
	elif "#" in g:
		print('\n' + g + '\n')
	elif "Could not find gadget for " not in g:
		print("ropchain += pack('<L', (%s)) %s %s [Line: %d]" % (g.split(':')[0], pound, g, linenumber))
